...
async function updateSupplierDiscount(name, discount) {
  ...
  await db.run('UPDATE proveedores SET descuento = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [discount || 0, proveedor.id]);
}
...
async function markUploadStatus(listaId, status, message = null) {
  const db = await getDb();
  await db.run('UPDATE listas_subidas SET estado = ?, mensaje = ? WHERE id = ?', [status, message, listaId]);
}
...
async function processPriceFile({ supplierName, discount = 0, originalName, tempFilePath }) {
  const db = await getDb();
  ...
  const listaId = await saveUploadRecord(proveedor.id, originalName, finalPath);

  try {
    const extracted = await extractCodesAndPrices(finalPath, proveedor.nombre);
    const discountFactor = 1 - (Number(discount || 0) / 100);

    await db.run('DELETE FROM precios_detectados WHERE lista_id = ?', [listaId]);
    ...
    for (const item of extracted) {
      ...
      const precioFinal = precio === null ? null : +(precio * discountFactor).toFixed(2);
      await db.run(
        ...
        [
          ...
          precio,
          precioFinal,
          String(item.confianza || 'media').trim().toLowerCase(),
          JSON.stringify(item),
        ]
      );
    }

    await markUploadStatus(listaId, 'procesado', `Se detectaron ${extracted.length} registros.`);
